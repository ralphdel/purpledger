...
export default function OnboardingPlanPage({ params }: OnboardingPageProps) {
  ...
  const [resolvedPlan, setResolvedPlan] = useState<"individual" | "corporate" | null>(null);
  const [email, setEmail] = useState("");
  const [businessName, setBusinessName] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Resolve plan from params on first render
  if (!resolvedPlan) {
    params.then(({ plan }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sessionRes = await fetch("/api/onboarding/create-session", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, businessName, plan: resolvedPlan }),
      });
  ...
  const Icon = resolvedPlan === "corporate" ? Building2 : User;
